...
async function main() {
  ...
  if (benchmarkIdx !== -1) { skipArgs.add(args[benchmarkIdx]); skipArgs.add(args[benchmarkIdx + 1]); }
  if (modelIdx !== -1) { skipArgs.add(args[modelIdx]); skipArgs.add(args[modelIdx + 1]); }
  const imagePath = args.find((a, i) => !a.startsWith("--") && !skipArgs.has(a))
    || "../ornek-captcha.png";
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    console.log(`  Run ${String(i + 1).padStart(2)}: ${answer.padEnd(8)}  ${pass ? "PASS" : "FAIL"}`);
  }
